...
#else
    // 横向き表示(画面右に配置)
    CGRect frame = CGRectMake(0.0f, 480-kIMAdViewDefaultHeight, kIMAdViewDefaultWidth, kIMAdViewDefaultHeight);
#endif

    //CGRect frame = CGRectMake(0.0f, 0.0f, kIMAdViewDefaultWidth, kIMAdViewDefaultHeight);

    IMobileAdView *imAdView = [[IMobileAdView alloc] initWithFrame:frame
                                             publisherId:YOUR_PUBLISHER_ID
                                                 mediaId:YOUR_MEDIA_ID
                                                  spotId:YOUR_SPOT_ID
                                                testMode:NO];      //テストモードフラグ
